...
<head>
    ...
    <style>
	 ...
	 /* #zycia{position:absolute;top:100px;left:50px; font: italic bold 2.5vmin verdena;}*/
      ...
      #a1 {position:absolute;left:45%;bottom:1%;width:10%;height:1%;border:1px solid black;background-color:red;}
    ...
    <script>
      ...
      i1.style.left=xx+"%";
	  i1.style.top=yy+"%";
      id1=setInterval("gs()",7);
    }
    var x=0.45*document.documentElement.clientWidth,stepSize=document.documentElement.clientWidth*0.01,test;
    function keyDown(e)
    {
	...
	function reset()
	{
		...
		j=1;
		pierwszeodbicie=0;
		x=0.45*document.documentElement.clientWidth;
		a1.style.left=x+"px";
		clearInterval(id1);
		startA();
	}
...
<input type="hidden" id="wynik" name="wynik"/>
  ...
  <h1 id="rekord"><?php if(isset($_SESSION['username'])){
	...
	$s="select max(sredni_wynik) as record from bounce where userLogin='{$_SESSION['username']}'";
