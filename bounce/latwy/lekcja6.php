...
<head>
    ...
    <style>
	 ...
	 /* #zycia{position:absolute;top:100px;left:50px; font: italic bold 2.5vmin verdena;}*/
      ...
      #a1 {position:absolute;left:42.5%;bottom:1%;width:15%;height:1%;border:1px solid black;background-color:red;}#rekord{position:absolute;top:17%;left:15%;font: italic bold 2.5vmin verdena;}
    </style>
    <script>
      ...
      i1.style.left=xx+"%";
	  i1.style.top=yy+"%";
      id1=setInterval("gs()",9);
    }
    var x=0.425*document.documentElement.clientWidth,stepSize=document.documentElement.clientWidth*0.01,test;
    function keyDown(e)
    {
	...
	if(zycia!=0){
	  ...
	  if(zycia==0)
	  {
		if(spr==0){
		spr=1;
		document.getElementById("wynik").value =Math.trunc(wynik/2);
	...
	function reset()
	{
		...
		j=1;
		pierwszeodbicie=0;
		x=0.425*document.documentElement.clientWidth;
		a1.style.left=x+"px";
		clearInterval(id1);
		startA();
	}
...
<input type="hidden" id="wynik" name="wynik"/>
  ...
  <h1 id="rekord"><?php if(isset($_SESSION['username'])){
	...
	$s="select max(latwy_wynik) as record from bounce where userLogin='{$_SESSION['username']}'";
